...
export const HeroesGridCard = ({heroe} : Props) => {

  const {isFavorite,toggleFavorite} = use(FavoriteHeroContext);
  
  const navigate = useNavigate();

  console.log("POWERS:", heroe.powers);
         ...
         <Card className="group overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-gradient-to-br from-white to-gray-50">
            <div className="relative h-64">
              ...
              <div className="absolute top-3 left-3 flex items-center gap-2">
                <div className={`w-3 h-3 rounded-full bg-green-500 ${heroe.status === "Active" ? 'bg-green-500' : 'bg-red-500'}`} />
              ...
              {/* Universe badge */}
              <Badge className={`absolute top-3 right-3 text-xs  text-white ${heroe.universe === "DC" ? "bg-blue-600" : "bg-red-600"}`}>{heroe.universe}</Badge>

              {/* Favorite button */}
              <Button size="sm" variant="ghost" className="absolute bottom-3 right-3 bg-white/90 hover:bg-white" onClick={()=>toggleFavorite(heroe)}>
